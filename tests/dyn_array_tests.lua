...
tl:group('dyn_array.new() -> [dyn_array]', function()
	tl:test('initializes an empty dyn_array with _len == 0', function()
...
tl:group('dyn_array:insert(e [any]) ->', function()
	tl:test('inserts an element e to the end of the dyn_array and increments _len by 1', function()
		local a = dyn_array.new()
		a:insert(1)
		a:insert(3)
...
tl:group('dyn_array:insert_at(e [any], i [int]) ->', function()
	tl:test('inserts an element e at index i in the dyn_array and increases _len by 1', function()
		local a = dyn_array.new()
		a:insert(4)
		a:insert(1)
		a:insert_at(2, 2)
...
tl:group('dyn_array:length() -> [int]', function()
	tl:test('returns the number of elements in the dyn_array, tracked by inst._len', function()
		local a = dyn_array.new()
		a:insert(1)
		a:insert(2)
		assert(a._len == 2)
	...
	tl:test('empty dyn_array returns 0', function()
		local a = dyn_array.new()
		assert(a._len == 0)
		assert(a:length() == 0)
	end)
...
tl:group('dyn_array:clear() ->', function()
...
tl:group('dyn_array:remove(e [any]) ->', function()
	...
	tl:test('removes the first element [number] found in dyn_array when there are multiple', function()
		local a = dyn_array.new()
		a:insert(1)
		a:insert(2)
		a:insert(1)
	...
	tl:test('removes the first element [table] found in dyn_array when there are multiple', function()
		local a = dyn_array.new()
		local pt1 = { x=0, y=0 }
		local pt2 = { x=1, y=0 }
		a:insert(pt1)
	...
	tl:test('removes the first element [string] found in dyn_array when there are multiple', function()
		local a = dyn_array.new()
		a:insert('hello')
		a:insert('world')
		a:insert('hello')
	...
	tl:test('removes the first element [boolean] found in dyn_array when there are multiple', function()
		local a = dyn_array.new()
		a:insert(true)
		a:insert(false)
		a:insert(true)
	...
	tl:test('does nothing when target element [int] is not in the dyn_array', function()
		local a = dyn_array.new()
		a:insert(1)
		a:insert(2)
	...
	tl:test('does nothing when target element [table] is not in the dyn_array', function()
		local a = dyn_array.new()
		local pt1 = {x=0}
		local pt2 = {x=1}
		a:insert(pt1)
...
tl:group('dyn_array:index_of(e [any], start_idx [int], end_idx [int]) -> [int]', function()
	local a = dyn_array.new({ 10, 2, -34, -133, 2, 7})
	
	tl:test('returns the first found index of the target element in the dyn_array', function()
		assert(a:index_of(2, 1, 6) == 2)
	end)

	tl:test('returns the index of the target element in the dyn_array within the specified bounds', function()
	...
	tl:test('returns -1 when the target element is not in the dyn_array', function()
		assert(a:index_of(65) == -1)
	end)

	tl:test('raises an error for start_idx = 0; index out of bounds', function()
		local result, err = pcall(function() return a:index_of(10, 0, 2) end)
		assert(not result and err:find('index out of bounds') ~= nil)
	end)

	tl:test('raises an error for start_idx = -1; index out of bounds', function()
		local result, err = pcall(function() return a:index_of(10, -1, 2) end)
		assert(not result and err:find('index out of bounds') ~= nil)
	end)

	tl:test('raises an error for start_idx > end_idx; index out of bounds', function()
		local result, err = pcall(function() return a:index_of(10, 2, 1) end)
		assert(not result and err:find('index out of bounds') ~= nil)
	end)

	tl:test('raises an error for end_idx > a._len; index out of bounds', function()
...
tl:group('dyn_array:contains(e [any]) -> [bool]', function()
...
tl:group('dyn_array:insert_range_at(arr [table], idx [int]) ->', function()
	tl:test('inserts dyn_array into target dyn_array at index idx', function()
...
tl:group('dyn_array:remove_at(idx [int]) ->', function()
...
tl:group('dyn_array:remove_range_at(start_idx [int], remove_count [int]) ->', function()
	tl:test('removes remove_count elements starting at start_idx', function()
...
tl:group('dyn_array:reverse() ->', function()
	tl:test('reverses the dyn_array', function()
...
tl:group('dyn_array:last_index_of(e [any], start_idx [int], end_idx [int]) -> [int]', function()
...
tl:group('dyn_array.__index(t [table], k [any]) -> [any]', function()
	...
	tl:test('raises an error for index k, k == 0; index out of bounds', function()
		local a = dyn_array.new()
		a:insert(1)
	...
	tl:test('raises an error for index k, k > inst._len; index out of bounds', function()
		local a = dyn_array.new()
		a:insert(2)
...
tl:group('dyn_array.__newindex(t [table], k [any], v [any]) ->', function()
	tl:test('raises an error for attempting to add properties onto the dyn_array', function()
		local a = dyn_array.new()
