...
ut:group('array.new() -> [array]', function()
	ut:test('initializes an empty array with _len == 0', function()
		local a = array.new()
		assert(a._len == 0)
...
ut:group('array:insert(e [any]) ->', function()
	ut:test('inserts an element e to the end of the array and increments _len by 1', function()
		local a = array.new()
		a:insert(1)
		a:insert(3)
...
ut:group('array:insert_at(e [any], i [int]) ->', function()
	ut:test('inserts an element e at index i in the array and increases _len by 1', function()
		local a = array.new()
		a:insert(4)
		a:insert(1)
		a:insert_at(2, 2)
...
ut:group('array:length() -> [int]', function()
	ut:test('returns the number of elements in the array, tracked by inst._len', function()
		local a = array.new()
		a:insert(1)
		a:insert(2)
		assert(a._len == 2)
	...
	ut:test('empty array returns 0', function()
		local a = array.new()
		assert(a._len == 0)
		assert(a:length() == 0)
	end)
...
ut:group('array:remove(e [any]) ->', function()
	...
	ut:test('removes the first element [number] found in array when there are multiple', function()
		local a = array.new()
		a:insert(1)
		a:insert(2)
		a:insert(1)
	...
	ut:test('removes the first element [table] found in array when there are multiple', function()
		local a = array.new()
		local pt1 = { x=0, y=0 }
		local pt2 = { x=1, y=0 }
		a:insert(pt1)
	...
	ut:test('removes the first element [string] found in array when there are multiple', function()
		local a = array.new()
		a:insert('hello')
		a:insert('world')
		a:insert('hello')
	...
	ut:test('removes the first element [boolean] found in array when there are multiple', function()
		local a = array.new()
		a:insert(true)
		a:insert(false)
		a:insert(true)
...
ut:group('array:index_of(e [any], start_idx [int], end_idx [int]) -> [int]', function()
	local a = array.new({ 10, 2, -34, -133, 2, 7})
	
	ut:test('returns the first found index of the target element in the array', function()
		assert(a:index_of(2, 1, 6) == 2)
	end)

	ut:test('returns the index of the target element in the array within the specified bounds', function()
	...
	ut:test('returns -1 when the target element is not in the array', function()
...
ut:group('array:contains(e [any]) -> [bool]', function()
	local a = array.new({ 1, -2, 13 })
...
ut:group('array:insert_range_at(arr [table], idx [int]) ->', function()
	ut:test('inserts array into target array at index idx', function()
		local a = array.new({ 23, -1, 2 })
		a:insert_range_at(array.new({2, 3, 4, 7}), 2)
		assert(
			a._len == 7 and
			a[1] == 23 and
			a[2] == 2 and
...
ut:group('array.__index(t [table], k [any]) -> [any]', function()
...
ut:group('array.__newindex(t [table], k [any], v [any]) ->', function()
	ut:test('raises an error for attempting to add properties onto the array', function()
		local a = array.new()
